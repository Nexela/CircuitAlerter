...
function Game.print_force_on_surface(force,surface, msg)
    local force_name
    if type(force) == "string" then
        force_name = force
    else
    ...
    return Game.print_all(msg, function(player)
        return (player.force.name == force_name and player.surface.name == surface_name)
    end)
end

--------------------------------------------------------------------------------------
--TODO Set goal for force
function Game.SetGoalForAllPlayers( goalText )
